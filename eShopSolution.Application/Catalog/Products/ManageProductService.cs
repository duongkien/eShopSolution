...
namespace eShopSolution.Application.Catalog.Products
{
    public class ManageProductService : IManageProductService
    {
        ...
        public ManageProductService(EShopDbContext contex)
        {
            _context = contex;
        }
        ...
        public async Task<PagedResult<ProductViewModel>> GetAllPageding(GetProductPagingRequest request)
        {
            //1. Select join
            ...
            var data = await query.Skip((request.PageIndex - 1) * request.PageIndex).Take(request.PageSize)
                .Select(x => new ProductViewModel()
                {
                    ...
                    Description = x.pt.Details,
                    LanguageId = x.pt.Description,
